...
use nalgebra::{
    DVector, Isometry3, Matrix3xX, Matrix4, Matrix6xX, Quaternion, Translation3, U3,
...
impl LinkState {
    ...
    pub fn get_linear_world_velocity(&self) -> Result<Vector3<f64>, Error> {
        match &self.world_velocity {
            None => Err(Error::new(
                "LinkState contains no velocity. You have to set compute_link_velocity to true in get_link_state() to get the velocity",
            )),
